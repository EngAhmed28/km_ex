
import React, { createContext, useContext, useState, useEffect } from 'react';
import { Language } from '../types';

interface LanguageContextType {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: (key: string) => string;
}

const translations: Record<Language, Record<string, string>> = {
  ar: {
    home: 'الرئيسية',
    shop: 'المتجر',
    categories: 'الأقسام',
    about: 'عن المتجر',
    login: 'تسجيل الدخول',
    signup: 'إنشاء حساب',
    logout: 'تسجيل الخروج',
    search: 'ابحث عن مكملك...',
    cart: 'السلة',
    heroTitle: 'أطلق الوحش الذي بداخلك',
    heroSub: 'أفضل المكملات الغذائية العالمية لبناء العضلات والأداء الرياضي المثالي.',
    shopNow: 'تسوق الآن',
    featured: 'أفضل المنتجات',
    newArrivals: 'وصل حديثاً',
    footerAbout: 'King of Muscles هو وجهتك الأولى للحصول على أجود أنواع المكملات الغذائية في العالم العربي.',
    rights: 'جميع الحقوق محفوظة © 2024',
    addToCart: 'أضف للسلة',
    bestSellers: 'الأكثر مبيعاً',
    reviews: 'تقييمات العملاء',
    price: 'السعر',
    weight: 'الوزن',
    flavor: 'النكهة',
    quantity: 'الكمية',
    description: 'الوصف',
    nutritionFacts: 'القيم الغذائية',
    similarProducts: 'منتجات مشابهة',
    checkout: 'إتمام الطلب',
    emptyCart: 'سلة المشتريات فارغة',
    total: 'الإجمالي',
    email: 'البريد الإلكتروني',
    password: 'كلمة المرور',
    forgotPassword: 'نسيت كلمة المرور؟',
    dontHaveAccount: 'ليس لديك حساب؟',
    haveAccount: 'لديك حساب بالفعل؟',
    confirmPassword: 'تأكيد كلمة المرور',
    filterBy: 'تصفية حسب',
    sortBy: 'ترتيب حسب',
    all: 'الكل',
    clearAll: 'مسح الكل',
    customSearch: 'بحث مخصص',
    searchHere: 'ابحث هنا...',
    wishlistOnly: 'المفضلة فقط',
    noResults: 'لا توجد نتائج',
    resetFilters: 'إعادة ضبط الفلاتر',
    orderSummary: 'ملخص الطلب',
    subtotal: 'المجموع الفرعي',
    shipping: 'الشحن',
    free: 'مجاناً',
    discountCode: 'كود الخصم',
    apply: 'تطبيق',
    addedSuccess: 'تمت الإضافة بنجاح!',
    checkoutNow: 'إنهاء الطلب الآن',
    original: 'أصلي 100%',
    fastDelivery: 'شحن سريع',
    easyReturn: 'إرجاع سهل',
    exclusiveOffers: 'عروض حصرية',
    promoTitle: 'احصل على خصم 20% على طلبك الأول',
    promoSub: 'اشترك في قائمتنا البريدية وكن أول من يعرف عن أحدث المكملات والعروض الحصرية.',
    subscribe: 'اشترك',
    fullName: 'الاسم بالكامل',
    welcomeBack: 'أهلاً بك في متجر الأبطال',
    loading: 'جاري التحميل...',
    viewDetails: 'عرض التفاصيل',
    bigOffer: 'عرض كبير',
    specialOffers: 'العروض الخاصة',
    support: 'الدعم الفني',
    faqs: 'الأسئلة الشائعة',
    returnPolicy: 'سياسة الإرجاع',
    shippingDelivery: 'الشحن والتوصيل',
    contactUs: 'اتصل بنا',
    connectWithUs: 'تواصل معنا',
    address: 'الرياض، المملكة العربية السعودية - شارع التحلية',
    shopByGoal: 'تسوق حسب هدفك',
    goalMuscle: 'بناء العضلات',
    goalWeight: 'خسارة الوزن',
    goalEnergy: 'الطاقة والتحمل',
    goalHealth: 'الصحة العامة',
    trustedBy: 'موثوق من قبل الأبطال',
    happyCustomers: 'عميل سعيد',
    originalProducts: 'منتج أصلي',
    yearsExperience: 'سنوات خبرة',
    ourBrands: 'شركاء النجاح',
    trustedBrands: 'البراندات العالمية الأكثر ثقة',
    latestNews: 'آخر الأخبار والنصائح',
    readMore: 'اقرأ المزيد',
    joinTribe: 'انضم إلى مجتمعنا على إنستغرام',
    dealOfTheDay: 'صفقة اليوم',
    massiveDeal: 'صفقة ضخمة على البروتين',
    endsIn: 'ينتهي خلال',
    hours: 'ساعة',
    minutes: 'دقيقة',
    seconds: 'ثانية',
    claimOffer: 'احصل على العرض الآن',
    // Admin Dashboard
    adminDashboard: 'لوحة تحكم المدير',
    welcomeAdmin: 'مرحباً',
    totalUsers: 'إجمالي المستخدمين',
    products: 'المنتجات',
    orders: 'الطلبات',
    adminCategories: 'الأقسام',
    overview: 'نظرة عامة',
    userManagement: 'إدارة المستخدمين',
    productManagement: 'إدارة المنتجات',
    categoryManagement: 'إدارة الأقسام',
    orderManagement: 'إدارة الطلبات',
    manageAllUsers: 'إدارة جميع مستخدمي النظام',
    manageAllCategories: 'إدارة جميع أقسام المتجر',
    manageAllProducts: 'إدارة جميع منتجات المتجر',
    manageAllOrders: 'إدارة جميع الطلبات',
    addCategory: 'إضافة قسم',
    addUser: 'إضافة مستخدم',
    addProduct: 'إضافة منتج',
    backToDashboard: 'العودة للداشبورد',
    searchCategory: 'ابحث عن قسم...',
    searchUser: 'ابحث عن مستخدم...',
    searchProduct: 'ابحث عن منتج...',
    nameArabic: 'الاسم (عربي)',
    nameEnglish: 'الاسم (إنجليزي)',
    adminDescription: 'الوصف',
    categoryImage: 'صورة القسم',
    activeCategory: 'قسم نشط',
    status: 'الحالة',
    active: 'نشط',
    inactive: 'معطل',
    actions: 'الإجراءات',
    edit: 'تعديل',
    delete: 'حذف',
    activate: 'تفعيل',
    deactivate: 'تعطيل',
    creationDate: 'تاريخ الإنشاء',
    role: 'الدور',
    adminEmail: 'البريد الإلكتروني',
    adminPassword: 'كلمة المرور',
    cancel: 'إلغاء',
    save: 'حفظ',
    saveChanges: 'حفظ التغييرات',
    addCategoryTitle: 'إضافة قسم جديد',
    editCategoryTitle: 'تعديل القسم',
    addProductTitle: 'إضافة منتج جديد',
    editProductTitle: 'تعديل المنتج',
    noCategories: 'لا يوجد أقسام',
    noUsers: 'لا يوجد مستخدمين',
    noProducts: 'لا يوجد منتجات',
    allRoles: 'جميع الأدوار',
    admin: 'مدير',
    employee: 'موظف',
    customer: 'عميل',
    manager: 'مدير',
    adminLoading: 'جاري التحميل...',
    errorLoading: 'فشل التحميل',
    networkError: 'خطأ في الشبكة',
    categoryAdded: 'تم إضافة القسم بنجاح',
    categoryUpdated: 'تم تحديث القسم بنجاح',
    categoryDeleted: 'تم حذف القسم بنجاح',
    userAdded: 'تم إضافة المستخدم بنجاح',
    userUpdated: 'تم تحديث المستخدم بنجاح',
    userDeleted: 'تم حذف المستخدم بنجاح',
    confirmDelete: 'هل أنت متأكد من الحذف؟',
    fillAllFields: 'يرجى ملء جميع الحقول المطلوبة',
    imageUploadFailed: 'فشل رفع الصورة',
    categoryCreateFailed: 'فشل إنشاء القسم',
    categoryUpdateFailed: 'فشل تحديث القسم',
    categoryDeleteFailed: 'فشل حذف القسم',
    productAdded: 'تم إضافة المنتج بنجاح',
    productUpdated: 'تم تحديث المنتج بنجاح',
    productDeleted: 'تم حذف المنتج بنجاح',
    productCreateFailed: 'فشل إنشاء المنتج',
    productUpdateFailed: 'فشل تحديث المنتج',
    productDeleteFailed: 'فشل حذف المنتج',
    productImage: 'صورة المنتج',
    productCategory: 'قسم المنتج',
    oldPrice: 'السعر القديم',
    stock: 'الكمية المتاحة',
    rating: 'التقييم',
    reviewsCount: 'عدد التقييمات',
    flavors: 'النكهات',
    nutrition: 'القيم الغذائية',
    addFlavor: 'إضافة نكهة',
    addNutrition: 'إضافة قيمة غذائية',
    nutritionLabelAr: 'التسمية (عربي)',
    nutritionLabelEn: 'التسمية (إنجليزي)',
    nutritionValue: 'القيمة',
    currentImage: 'الصورة الحالية',
    // Checkout page
    doYouHaveAccount: 'هل لديك حساب؟',
    checkoutLogin: 'تسجيل الدخول',
    createAccount: 'إنشاء حساب',
    loginToAutoFill: 'سجل دخولك لتعبئة بياناتك تلقائياً ومتابعة جميع فواتيرك',
    canCompleteWithoutLogin: 'يمكنك إتمام الطلب بدون التسجيل دخول، لكن تسجيل الدخول يساعدك في متابعة جميع فواتيرك',
    billingDetails: 'تفاصيل الفاتورة',
    fullNameRequired: 'الاسم ثلاثي *',
    enterFullName: 'ادخل الاسم الكامل',
    countryRegion: 'الدولة / المنطقة *',
    egypt: 'مصر',
    governorate: 'المحافظة *',
    enterGovernorate: 'أدخل المحافظة',
    cityStreet: 'مدينة / شارع *',
    enterCityStreet: 'مدينة / مركز / اسم الشارع أو الحي',
    checkoutPhone: 'الهاتف *',
    enterPhone: '01XXXXXXXXX',
    backupPhone: 'رقم آخر احتياطي (اختياري)',
    checkoutEmail: 'البريد الإلكتروني (اختياري)',
    enterEmail: 'example@email.com',
    orderNotes: 'ملاحظات الطلب (اختياري)',
    enterOrderNotes: 'ملاحظات حول الطلب مثال ملحوظة خاصة بتسليم الطلب',
    yourOrder: 'طلبك',
    checkoutQuantity: 'الكمية',
    governorateShipping: 'شحن المحافظات',
    advancePayment: 'يتم تحويل 100 جنيه مقدماً',
    transferNumber: 'رقم التحويل:',
    sendTransferOnWhatsapp: 'إرسال صورة التحويل على واتساب نفس الرقم',
    shippingWillUpdate: 'سيتم تحديث خيارات الشحن أثناء السداد.',
    paymentMethod: 'طريقة الدفع',
    vodafoneCash: 'فودافون كاش',
    instapay: 'انستاباي',
    cashOnDelivery: 'الدفع نقداً عند الاستلام',
    governorateShippingInfo: 'يتم تحويل 100 جنيه فودافون كاش مقدماً تأكيد للدور در تخصم من ثمن الأوردر على الرقم 03000000000 والتأكيد على الواتساب',
    privacyPolicy: 'سياسة الخصوصية',
    privacyPolicyText: 'Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our',
    confirmOrder: 'تأكيد الطلب',
    proceedToComplete: 'التقدم لإتمام الطلب',
    orderPlacedSuccessfully: 'تم تأكيد الطلب بنجاح',
    orderNumber: 'رقم الطلب',
    thankYouMessage: 'شكراً لك على طلبك! سنقوم بالتواصل معك قريباً.',
    continueShopping: 'متابعة التسوق',
  },
  en: {
    home: 'Home',
    shop: 'Shop',
    categories: 'Categories',
    about: 'About',
    login: 'Login',
    signup: 'Sign Up',
    logout: 'Logout',
    search: 'Search for supplements...',
    cart: 'Cart',
    heroTitle: 'Unleash Your Inner Beast',
    heroSub: 'Premium global supplements for muscle building and peak athletic performance.',
    shopNow: 'Shop Now',
    featured: 'Featured Products',
    newArrivals: 'New Arrivals',
    footerAbout: 'King of Muscles is your primary destination for the highest quality supplements in the Arab world.',
    rights: 'All Rights Reserved © 2024',
    addToCart: 'Add to Cart',
    bestSellers: 'Best Sellers',
    reviews: 'Customer Reviews',
    price: 'Price',
    weight: 'Weight',
    flavor: 'Flavor',
    quantity: 'Quantity',
    description: 'Description',
    nutritionFacts: 'Nutrition Facts',
    similarProducts: 'Similar Products',
    checkout: 'Checkout',
    emptyCart: 'Cart is empty',
    total: 'Total',
    email: 'Email',
    password: 'Password',
    forgotPassword: 'Forgot Password?',
    dontHaveAccount: "Don't have an account?",
    haveAccount: 'Already have an account?',
    confirmPassword: 'Confirm Password',
    filterBy: 'Filter By',
    sortBy: 'Sort By',
    all: 'All',
    clearAll: 'Clear All',
    customSearch: 'Custom Search',
    searchHere: 'Search here...',
    wishlistOnly: 'Wishlist Only',
    noResults: 'No Results Found',
    resetFilters: 'Reset Filters',
    orderSummary: 'Order Summary',
    subtotal: 'Subtotal',
    shipping: 'Shipping',
    free: 'Free',
    discountCode: 'Discount Code',
    apply: 'Apply',
    addedSuccess: 'Added Successfully!',
    checkoutNow: 'Checkout Now',
    original: '100% Original',
    fastDelivery: 'Fast Delivery',
    easyReturn: 'Easy Returns',
    exclusiveOffers: 'Exclusive Offers',
    promoTitle: 'Get 20% Off Your First Order',
    promoSub: 'Subscribe to our newsletter and be the first to know about the latest supplements and offers.',
    subscribe: 'Subscribe',
    fullName: 'Full Name',
    welcomeBack: 'Welcome to the Champions Store',
    loading: 'Loading...',
    viewDetails: 'View Details',
    bigOffer: 'Big Offer',
    specialOffers: 'Special Offers',
    support: 'Support',
    faqs: 'FAQs',
    returnPolicy: 'Return Policy',
    shippingDelivery: 'Shipping & Delivery',
    contactUs: 'Contact Us',
    connectWithUs: 'Connect With Us',
    address: 'Riyadh, Saudi Arabia - Tahlia St.',
    shopByGoal: 'Shop by Goal',
    goalMuscle: 'Build Muscle',
    goalWeight: 'Lose Weight',
    goalEnergy: 'Energy & Focus',
    goalHealth: 'General Health',
    trustedBy: 'Trusted by Champions',
    happyCustomers: 'Happy Customers',
    originalProducts: 'Original Products',
    yearsExperience: 'Years Experience',
    ourBrands: 'Our Brands',
    trustedBrands: 'Most Trusted Global Brands',
    latestNews: 'Latest Tips & News',
    readMore: 'Read More',
    joinTribe: 'Join the Tribe on Instagram',
    dealOfTheDay: 'Deal of the Day',
    massiveDeal: 'MASSIVE DEAL ON WHEY',
    endsIn: 'Ends in',
    hours: 'Hrs',
    minutes: 'Mins',
    seconds: 'Secs',
    claimOffer: 'Claim Offer Now',
    // Admin Dashboard
    adminDashboard: 'Admin Dashboard',
    welcomeAdmin: 'Welcome',
    totalUsers: 'Total Users',
    products: 'Products',
    orders: 'Orders',
    adminCategories: 'Categories',
    overview: 'Overview',
    userManagement: 'User Management',
    productManagement: 'Product Management',
    categoryManagement: 'Category Management',
    orderManagement: 'Order Management',
    manageAllUsers: 'Manage all system users',
    manageAllCategories: 'Manage all store categories',
    manageAllProducts: 'Manage all store products',
    manageAllOrders: 'Manage all orders',
    addCategory: 'Add Category',
    addUser: 'Add User',
    addProduct: 'Add Product',
    backToDashboard: 'Back to Dashboard',
    searchCategory: 'Search for category...',
    searchUser: 'Search for user...',
    searchProduct: 'Search for product...',
    nameArabic: 'Name (Arabic)',
    nameEnglish: 'Name (English)',
    adminDescription: 'Description',
    categoryImage: 'Category Image',
    activeCategory: 'Active Category',
    status: 'Status',
    active: 'Active',
    inactive: 'Inactive',
    actions: 'Actions',
    edit: 'Edit',
    delete: 'Delete',
    activate: 'Activate',
    deactivate: 'Deactivate',
    creationDate: 'Creation Date',
    role: 'Role',
    adminEmail: 'Email',
    adminPassword: 'Password',
    cancel: 'Cancel',
    save: 'Save',
    saveChanges: 'Save Changes',
    addCategoryTitle: 'Add New Category',
    editCategoryTitle: 'Edit Category',
    addProductTitle: 'Add New Product',
    editProductTitle: 'Edit Product',
    noCategories: 'No categories',
    noUsers: 'No users',
    noProducts: 'No products',
    allRoles: 'All Roles',
    admin: 'Admin',
    employee: 'Employee',
    customer: 'Customer',
    manager: 'Manager',
    adminLoading: 'Loading...',
    errorLoading: 'Failed to load',
    networkError: 'Network error',
    categoryAdded: 'Category added successfully',
    categoryUpdated: 'Category updated successfully',
    categoryDeleted: 'Category deleted successfully',
    userAdded: 'User added successfully',
    userUpdated: 'User updated successfully',
    userDeleted: 'User deleted successfully',
    confirmDelete: 'Are you sure you want to delete?',
    fillAllFields: 'Please fill all required fields',
    imageUploadFailed: 'Failed to upload image',
    categoryCreateFailed: 'Failed to create category',
    categoryUpdateFailed: 'Failed to update category',
    categoryDeleteFailed: 'Failed to delete category',
    productAdded: 'Product added successfully',
    productUpdated: 'Product updated successfully',
    productDeleted: 'Product deleted successfully',
    productCreateFailed: 'Failed to create product',
    productUpdateFailed: 'Failed to update product',
    productDeleteFailed: 'Failed to delete product',
    productImage: 'Product Image',
    productCategory: 'Product Category',
    oldPrice: 'Old Price',
    stock: 'Stock',
    rating: 'Rating',
    reviewsCount: 'Reviews Count',
    flavors: 'Flavors',
    nutrition: 'Nutrition',
    addFlavor: 'Add Flavor',
    addNutrition: 'Add Nutrition',
    nutritionLabelAr: 'Label (Arabic)',
    nutritionLabelEn: 'Label (English)',
    nutritionValue: 'Value',
    currentImage: 'Current Image',
    // Checkout page
    doYouHaveAccount: 'Do you have an account?',
    checkoutLogin: 'Login',
    createAccount: 'Create Account',
    loginToAutoFill: 'Log in to automatically fill your data and track all your invoices',
    canCompleteWithoutLogin: 'You can complete the order without logging in, but logging in helps you track all your invoices',
    billingDetails: 'Billing Details',
    fullNameRequired: 'Full Name *',
    enterFullName: 'Enter full name',
    countryRegion: 'Country / Region *',
    egypt: 'Egypt',
    governorate: 'Governorate *',
    enterGovernorate: 'Enter Governorate',
    cityStreet: 'City / Street *',
    enterCityStreet: 'City / Center / Street Name or Neighborhood',
    checkoutPhone: 'Phone *',
    enterPhone: '01XXXXXXXXX',
    backupPhone: 'Another backup number (optional)',
    checkoutEmail: 'Email (optional)',
    enterEmail: 'example@email.com',
    orderNotes: 'Order Notes (optional)',
    enterOrderNotes: 'Notes about the order, e.g., a special note for delivery',
    yourOrder: 'Your Order',
    checkoutQuantity: 'Quantity',
    governorateShipping: 'Governorate Shipping',
    advancePayment: 'An advance payment of 100 EGP is required',
    transferNumber: 'Transfer Number:',
    sendTransferOnWhatsapp: 'Send a picture of the transfer via WhatsApp to the same number',
    shippingWillUpdate: 'Shipping options will be updated during payment.',
    paymentMethod: 'Payment Method',
    vodafoneCash: 'Vodafone Cash',
    instapay: 'InstaPay',
    cashOnDelivery: 'Cash on Delivery',
    governorateShippingInfo: '100 EGP Vodafone Cash to be transferred in advance as confirmation for the order, deducted from the order price on number 03000000000 and confirmed on WhatsApp',
    privacyPolicy: 'Privacy Policy',
    privacyPolicyText: 'Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our',
    confirmOrder: 'Confirm Order',
    proceedToComplete: 'Proceed to complete the order',
    orderPlacedSuccessfully: 'Order placed successfully',
    orderNumber: 'Order Number',
    thankYouMessage: 'Thank you for your order! We will contact you soon.',
    continueShopping: 'Continue Shopping',
  }
};

const LanguageContext = createContext<LanguageContextType | undefined>(undefined);

export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [language, setLang] = useState<Language>('ar');

  const setLanguage = (lang: Language) => {
    setLang(lang);
    document.documentElement.dir = lang === 'ar' ? 'rtl' : 'ltr';
    document.documentElement.lang = lang;
  };

  useEffect(() => {
    document.documentElement.dir = language === 'ar' ? 'rtl' : 'ltr';
  }, []);

  const t = (key: string) => translations[language][key] || key;

  return (
    <LanguageContext.Provider value={{ language, setLanguage, t }}>
      <div dir={language === 'ar' ? 'rtl' : 'ltr'} className={language === 'ar' ? 'font-ar' : 'font-en'}>
        {children}
      </div>
    </LanguageContext.Provider>
  );
};

export const useLanguage = () => {
  const context = useContext(LanguageContext);
  if (!context) throw new Error('useLanguage must be used within LanguageProvider');
  return context;
};
